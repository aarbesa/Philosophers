...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2023/10/26 10:48:21 by aabdyli           #+#    #+#             */
/*   Updated: 2023/10/26 10:48:24 by aabdyli          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void		ft_mutexes(t_room *room);
...
static void	ft_mutexes(t_room *room)
{
	...
	pthread_mutex_init(&room->write_mutex, NULL);
	i = 0;
	while (i < room->nb_philos)
	{
	...
	while (i < room->nb_philos)
	{
		...
		philo->fork = &room->forks[i];
		philo->fork2 = &room->forks[(i + 1) % room->nb_philos];
		i++;
	}
}
